...
2. 克隆repository
	...
	- `git clone -b <branch> <remote>` 克隆远程仓库的某个分支`remote`为本地的`branch`分支
...
2. 创建分支
	...
	- `git checkout -b 分支名 远程分支名` 相当于检出远程的某个分支到本地，远程分支名如origin/b1、remotes/origin/b1
		- `git checkout 远程分支名` 命令会是当前HEAD变为一个游离的HEAD（即现在HEAD指向的是一个没有分支名字的修订版本，游离于已知的所有分支之外，如`HEAD detached at origin/b1`）
